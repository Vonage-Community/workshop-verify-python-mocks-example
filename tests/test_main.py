...
class TestCheckCode(unittest.TestCase):
    ...
    def test_check_code_success(self, mock_check_code):
        """
        Test to see if a `status: complete` from Vonage
        results in rendering the success.html template
        """
        # Data to use in test
        test_data = {
        ...
        # Instantiate a real CheckCodeResponse object for the mock to return
        test_check_code_response = CheckCodeResponse(
            request_id="test-request-id", status="completed"
        )
        ...
        self.assertEqual(
            test_result.status_code,
            expected_result,
            msg=f"test_check_code_success failed with: {test_result.status_code}. Expected: {expected_result}",
        )
        mock_check_code.assert_called_once_with(
            request_id="test-request-id", code=test_data["code"]
        )
        self.assertNotIn("Invalid verification code.", test_result.text)
        self.assertNotIn("Invalid code. Please try again.", test_result.text)
        self.assertNotIn("Something went wrong. Please try again.", test_result.text)

    def test_check_code_failure_session_expired(self):
        """
        Test to see if an expired session
        results in rendering the verify.html template with appropriate error message
        """
        ...
        self.assertEqual(
            test_result.status_code,
            expected_result,
            msg=f"test_check_code_failure_session_expired failed with: {test_result.status_code}. Expected: {expected_result}",
        )
    ...
    def test_check_code_failure_authentication_error(self, mock_check_code):
        ...
        mock_check_code.side_effect = make_auth_error()
        ...
        self.assertEqual(
            test_result.status_code,
            expected_result,
            msg=f"test_check_code_failure_authentication_error failed with: {test_result.status_code}. Expected: {expected_result}",
        )
    ...
    def test_check_code_failure_http_request_error_400(self, mock_check_code):
        ...
        mock_check_code.side_effect = make_http_error(400)
        ...
        self.assertEqual(
            test_result.status_code,
            expected_result,
            msg=f"test_check_code_failure_http_request_error_400 failed with: {test_result.status_code}. Expected: {expected_result}",
        )
    ...
    def test_check_code_failure_http_request_error_4xx(self, mock_check_code):
        """
        Test to see if any other 400 HttpError from Vonage
        results in rendering the verify.html template with appropriate error message
        """
        ...
        mock_check_code.side_effect = make_http_error(401)
        ...
        self.assertEqual(
            test_result.status_code,
            expected_result,
            msg=f"test_check_code_failure_http_request_error_4xx failed with: {test_result.status_code}. Expected: {expected_result}",
        )
        self.assertIn("Something went wrong. Please try again.", test_result.text)
